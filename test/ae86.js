...
describe("ae86 - generate", function () {
  ...
  it("should copy static files and process pages", function (done) {
    sinon.stub(process, "cwd").value(function () {
      return p.join(DIRNAME, "/fixtures");
    });
    this.mockFs
      .expects("existsSync")
      .once()
      .withArgs(p.join(DIRNAME, "/fixtures", "/params.js"))
      .returns(true);
    this.mockCpr
      .expects("cpr")
      .once()
    ...
    const compileStub = sinon
      .stub(Engine.prototype, "compile")
      .value(function (dir, cb) {
        assert.isTrue(["partials", "layouts", "pages"].indexOf(dir) !== -1);
        cb();
      });
    const mergeStub = sinon
      .stub(Engine.prototype, "merge")
      .value(function (dir, templates, params, cb) {
  ...
  it("should handle invalid params file", function (done) {
    sinon.stub(process, "cwd").value(function () {
      return p.join(DIRNAME, "/fixtures-error");
    });
    this.mockCpr
      .expects("cpr")
      .once()
...
describe("ae86 - watch", function () {
  ...
  it("should ignore swap files and set sample rate on project directories and files", function (done) {
    ...
    sinon.stub(watch, "watchTree").value(function (file, cb) {
      assert.isTrue(
        ["static", "partials", "layouts", "pages", "params.js"].indexOf(
          file,
        ) !== -1,
      );
      watchCounter += 1;
  ...
  it("should log message when watching for file changes", function () {
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("Watching for file changes at %s...", "static");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("Watching for file changes at %s...", "partials");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("Watching for file changes at %s...", "layouts");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("Watching for file changes at %s...", "pages");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("Watching for file changes at %s...", "params.js");
    sinon.stub(watch, "watchTree").value(function (file, cb) {
  ...
  it("should log message and set listener when a file is created", function () {
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was created", "static");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was created", "partials");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was created", "layouts");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was created", "pages");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was created", "params.js");
    sinon.stub(watch, "watchTree").value(function (file, cb) {
      cb("somef", {}, null);
    });
    this.ae86.generate = sinon.spy();
  ...
  it("should log message and set listener when a file is modified", function () {
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was modified", "static");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was modified", "partials");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was modified", "layouts");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was modified", "pages");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was modified", "params.js");
    sinon.stub(watch, "watchTree").value(function (file, cb) {
      cb("somef", {}, {});
    });
    this.ae86.generate = sinon.spy();
  ...
  it("should log message and set listener when a file is deleted", function () {
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was deleted", "static");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was deleted", "partials");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was deleted", "layouts");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was deleted", "pages");
    this.mockBag
      .expects("logStepItemSuccess")
      .once()
      .withExactArgs("%s was deleted", "params.js");
    sinon.stub(watch, "watchTree").value(function (file, cb) {
      cb("somef", { nlink: 0 }, {});
    });
    this.ae86.generate = sinon.spy();
