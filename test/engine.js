...
describe("engine - compile", function () {
  ...
  it("should compile files with the specified file extension", function (done) {
    this.mockFs
      .expects("readFile")
      .withArgs("/somebasedir/foo.html", "utf8")
      .callsArgWith(2, null, "foo");
    this.mockFs
      .expects("readFile")
      .withArgs("/somebasedir/sub/dir/bar.html", "utf8")
    ...
    sinon.stub(f, "walkSync").value(function (dir, cb) {
      ...
      cb(
        "/somebasedir",
        [],
        ["foo.html", "sub/dir/bar.html", ".git", "abc.txt"],
      );
    });
...
describe("engine - merge", function () {
  ...
  it("should merge partials in page templates with non-default template", function (done) {
    const templates = {
        ...
        layouts: {
          "somelayout.html": jazz.compile(
            '<html lang="en"><head><title>Some Title</title><body>{content}</body></html>',
          ),
        },
      },
      params = {
    ...
    this.mockBag
      ...
      .withExactArgs("creating someoutputdir/page.html");
    sinon.stub(fs, "writeFile").value(function (page, content, encoding, cb) {
      assert.equals(page, "someoutputdir/page.html");
      assert.equals(
        content,
        '<html lang="en"><head><title>Some Title</title><meta name="generator" content="AE86 1.2.3"></head><body>Some footer text</body></html>',
      );
  ...
  it("should merge layouts in page templates", function (done) {
    const templates = {
        ...
        layouts: {
          "default.html": jazz.compile(
            '<html lang="en"><head><title>Some Title</title><body>Some layout {content}</body></html>',
          ),
        },
      },
      params = {},
    ...
    this.mockBag
      .expects("logStepItemSuccess")
      .withExactArgs(util.format("creating someoutputdir/page.html"));
    sinon.stub(fs, "writeFile").value(function (page, content, encoding, cb) {
      assert.equals(page, "someoutputdir/page.html");
      assert.equals(
        content,
        '<html lang="en"><head><title>Some Title</title><meta name="generator" content="AE86 1.2.3"></head><body>Some layout Some content</body></html>',
      );
      assert.equals(encoding, "utf8");
      cb();
    });
  ...
  it("should merge parameters in page templates and pass any error", function (done) {
    const templates = {
        ...
        layouts: {
          "default.html": jazz.compile(
            '<html lang="en"><head><title>Some Title</title></head><body>{content}</body></html>',
          ),
        },
      },
      params = { foo: "bar" },
    ...
    sinon.stub(fs, "writeFile").value(function (page, content, encoding, cb) {
      ...
      assert.equals(
        content,
        '<html lang="en"><head><title>Some Title</title><meta name="generator" content="AE86 1.2.3"></head><body>Some content with param bar</body></html>',
      );
      assert.equals(encoding, "utf8");
  ...
  it("should log command message when there is no error while writing the page file", function (done) {
    ...
    this.mockBag
      .expects("logStepItemSuccess")
      .withExactArgs("creating some/output/dir/page.html");
    sinon.stub(fs, "writeFile").value(function (page, content, encoding, cb) {
