...
Engine.prototype._predefined = function (partials, params) {
    ...
    return {
        'include': function (file, cb) {
            partials[file].process(that._combine([params, that._predefined(partials, params)]), cb);
        }
    };
};
Engine.prototype.merge = function (page, layout, partials, params, cb) {
    var that = this;
    page.process(this._combine([params, this._predefined(partials, params)]), function (data) {
        layout.process(that._combine([params, that._predefined(partials, params), { body: data }]), cb);
    });
};
